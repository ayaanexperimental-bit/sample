...
async function ensureAdminUsersRoleSchema(db: D1Database) {
  const row = await db
    .prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'admin_users' LIMIT 1")
    .first<{ sql?: string | null }>();
  const sql = row?.sql || "";
  if (!sql) return;
  if (sql.includes("'admin'") && sql.includes("'super_admin'") && sql.includes("'inactive'")) return;

  const nextTable = `admin_users_next_${Date.now()}`;
  const oldTable = `admin_users_old_${Date.now()}`;
  ...
  await db.prepare("DROP INDEX IF EXISTS idx_admin_users_status").run();
  await db.prepare(`ALTER TABLE admin_users RENAME TO ${oldTable}`).run();
  await db.prepare(`ALTER TABLE ${nextTable} RENAME TO admin_users`).run();
  await db.prepare(`DROP TABLE ${oldTable}`).run();
